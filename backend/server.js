...
app.post("/api/inventario", (req, res) => {
  ...
  const query =
    "INSERT INTO inventario (name, quantity, weight, price, supplier) VALUES (?, ?, ?, ?, ?)";
  db.query(query, [name, quantity, weight, price, supplier], (err, result) => {
    if (err) {
      ...
      res
        .status(201)
        .json({ id: result.insertId, name, quantity, weight, price, supplier });
    }
  });
});
...
app.put("/api/inventario/:id", (req, res) => {
  ...
  const { name, quantity, weight, price, supplier } = req.body;
  const query =
    "UPDATE inventario SET name = ?, quantity = ?, weight = ?, price = ?, supplier = ? WHERE id = ?";
